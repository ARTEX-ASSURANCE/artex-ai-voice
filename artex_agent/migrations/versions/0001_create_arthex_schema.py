...
revision: str = '0001_arthex_initial_schema'
...
def upgrade() -> None:
    ...
    # ### SinistresArthex table ###
    op.create_table('sinistres_arthex',
        sa.Column('id_sinistre_arthex', sa.Integer(), autoincrement=True, nullable=False),
        ...
        sa.Column('statut_sinistre_arthex', sa.String(length=100), server_default='Information enregistrée par agent', nullable=False),
        sa.Column('date_survenance', sa.Date(), nullable=True),
        sa.Column('date_creation', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False),
        sa.Column('date_modification', sa.DateTime(), server_default=sa.text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), nullable=False),
        sa.ForeignKeyConstraint(['id_contrat'], ['contrats.id_contrat'], name=op.f('fk_sinistres_arthex_id_contrat_contrats')),
        sa.ForeignKeyConstraint(['id_adherent'], ['adherents.id_adherent'], name=op.f('fk_sinistres_arthex_id_adherent_adherents')),
        sa.PrimaryKeyConstraint('id_sinistre_arthex', name=op.f('pk_sinistres_arthex'))
    )
...
def downgrade() -> None:
    ...
    op.drop_table('sinistres_arthex')
